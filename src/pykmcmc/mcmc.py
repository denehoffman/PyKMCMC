...
def load_moves(config):
    ...
    for move_data in config:
        ...
        try:
            ...
            try:
                move_class = getattr(pykmcmc.moves, move_name)
            except:
                print(f"Error, {move_name} not found")
                sys.exit(1)
...
weight = 0.1
    ...
    if int(args["--ncores"]) > 1 or args["--mpi"]:
        ...
        pool = schwimmbad.choose_pool(
            mpi=args["--mpi"], processes=int(args["--ncores"])
        )
    ...
    if int(args["--ncores"]) > 1:
        run_table.add_row(
            "Multithreaded?", f"{bool(pool)} ({int(args['--ncores'])} cores)"
        )
    else:
    ...
    try:
        ...
        for _ in sampler.sample(
            ...
            progress_kwargs=dict(
                unit="step",
                ascii=["\x1b[38;5;237m━\x1b[0m", "\x1b[38;5;10m━\x1b[0m"],
                dynamic_ncols=True,
            ),
        ):
